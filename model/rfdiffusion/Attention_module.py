...
from model.rfdiffusion.util_module import init_lecun_normal
...
class MSARowAttentionWithBias(nn.Module):
    ...
    def forward(self, msa, pair, attention_mask=None): # TODO: make this as tied-attention
        B, N, L = msa.shape[:3]
        #
        ...
        if attention_mask is not None:
            # attention_mask: (B, L), True for valid residues
            # 1. 创建 pair-wise 掩码: (B, L, L)
            pair_mask = attention_mask[:, :, None] * attention_mask[:, None, :]
            
            # 2. 转换成 bias 添加项。无效位置 (False) 应该加上一个很大的负数
            mask_value = -1e9
            # (1 - pair_mask.float()) 会让无效位置为1，有效位置为0
            bias_adder = (1 - pair_mask.float()) * mask_value
            
            # 3. 扩展维度以匹配 bias 的形状 (B, L, L, h) 并相加
            bias = bias + bias_adder.unsqueeze(-1)
        # --- 修改结束 ---
...
class BiasedAxialAttention(nn.Module):
    ...
    def forward(self, pair, bias, attention_mask=None):
        # pair: (B, L, L, d_pair)
        B, L = pair.shape[:2]

        # 1. 如果提供了 attention_mask (B, L)，先用它创建正确的 pair-wise mask (B, L, L)
        pair_mask = None
        if attention_mask is not None:
            pair_mask = attention_mask[:, :, None] * attention_mask[:, None, :]

        # 2. 如果是行注意力，将所有需要置换的张量一起置换
        if self.is_row:
            pair = pair.permute(0, 2, 1, 3)
            bias = bias.permute(0, 2, 1, 3)
            if pair_mask is not None:
                # 同样置换 pair_mask 以保持对齐
                pair_mask = pair_mask.permute(0, 2, 1)
        ...
        bias_attn = self.to_b(bias) # 重命名以避免与输入 bias 混淆
        
        # 3. 将 attention mask (现在是 pair_mask) 应用到 attention bias 上
        if pair_mask is not None:
            mask_value = -1e9
            # (1 - pair_mask.float()) 会让无效位置为1，有效位置为0
            bias_adder = (1 - pair_mask.float()) * mask_value
            # 扩展维度以匹配 bias 的形状 (B, L, L, h) 并相加
            bias_attn = bias_attn + bias_adder.unsqueeze(-1)
        ...
        attn = attn + bias_attn # apply bias
