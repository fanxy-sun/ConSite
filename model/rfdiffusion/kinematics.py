import numpy as np
import torch
from model.rfdiffusion.chemical import INIT_CRDS
from model.rfdiffusion.util import generate_Cbeta
...
def xyz_to_c6d(xyz, params=PARAMS):
    ...
    Returns
        c6d: 存储了残基对之间的距离和三个方向角 (dist, omega, theta, phi)。形状: (B*T, L, L, 4)
        mask: 标记了哪些残基对的距离小于DMAX (20Å)，因此计算了方向角。形状: (B*T, L, L)
    """
...
def xyz_to_t2d(xyz_t, params=PARAMS):
    ...
    """
    B, T, L = xyz_t.shape[:3]
    # .view(B*T, L, 3, 3) -> 将批次和模板维度合并，方便批量处理
    # 输入 xyz_to_c6d 的张量形状为 (B*T, L, 3, 3)
    c6d, mask = xyz_to_c6d(xyz_t[:,:,:,:3].view(B*T,L,3,3), params=params)
    c6d = c6d.view(B, T, L, L, 4)
    mask = mask.view(B, T, L, L, 1)


    # c6d[...,0] 是距离信息，形状为 (B, T, L, L)
    # dist_to_onehot 将连续的距离值转换为离散的、独热编码的类别
    # 根据默认参数，距离被分到36个bin中，再加上一个表示距离过远的bin，总共37个类别
    # dist 形状: (B, T, L, L, 37)
    dist = dist_to_onehot(c6d[...,0], params)

    # c6d[...,1:] 是三个方向角 (omega, theta, phi)，形状为 (B, T, L, L, 3)
    # 对每个角度计算 sin 和 cos 值，将每个角度的表征从1维扩展到2维
    # torch.sin(...) 和 torch.cos(...) 的形状都是 (B, T, L, L, 3)
    # orien: torch.cat(...) 将 sin 和 cos 拼接起来，得到形状 (B, T, L, L, 6)
    # *mask -> mask，将距离过远的残基对的方向角信息置为0
    orien = torch.cat((torch.sin(c6d[...,1:]), torch.cos(c6d[...,1:])), dim=-1)*mask # (B, T, L, L, 6)

    mask = ~torch.isnan(c6d[:,:,:,:,0]) # mask: (B, T, L, L)
