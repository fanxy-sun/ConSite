...
class SelfAttention(nn.Module):
    def __init__(self, dim, k_dim=None, heads = 8, local_heads = 0, local_window_size = 256, nb_features = None, feature_redraw_interval = 1000, generalized_attention = False, kernel_fn = nn.ReLU(inplace=True), qr_uniform_q = False, dropout = 0., no_projection = False):
        ...
        self.max_tokens = 2**19  # $$注意这里控制内存使用，在L=600，BATCH=6，self.max_tokens = 2**16时，要循环约32次。循环次数约为 BATCH*L*L/self.max_tokens
    ...
    def forward(self, query, key, value, padding_mask=None, **kwargs):
        """
        Args:
            query, key, value (torch.Tensor): 输入张量
            padding_mask (torch.Tensor, optional): 形状为 (B, L) 或 (B*N, L) 的填充掩码。'True' 表示真实残基, 'False' 表示填充。Defaults to None.
        """
        ...
        if padding_mask is not None:
            ...
            # 这样它可以广播到 k/v 的 (b, h, n, d) 形状
            mask = padding_mask.unsqueeze(1).unsqueeze(-1)
            
            # 将 padding 位置的 k 和 v 向量置零
            # 我们用 ~mask 是因为掩码中 True 代表保留，False 代表置零
