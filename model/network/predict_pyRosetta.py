...
from model.network.parsers import parse_a3m, read_templates
from model.network.RoseTTAFoldModel import RoseTTAFoldModule
from model.network import util
from collections import namedtuple
from model.network.ffindex import *
from model.network.kinematics import xyz_to_c6d, c6d_to_bins2, xyz_to_t2d
...
class Predictor():
    ...
    def predict(self, a3m_fn, out_prefix, hhr_fn=None, atab_fn=None, window=150, shift=50):
        """
        Args:
            a3m_fn (str): 输入的多序列比对（MSA）文件路径，格式为 a3m。这是必需的输入。
            out_prefix (str): 输出文件的前缀。最终的输出文件将被命名为 [out_prefix].npz。
            hhr_fn (str, optional): HHsearch 的输出文件（.hhr 格式）。用于提供模板（template）信息。默认为 None。
            atab_fn (str, optional): HHsearch 的另一种输出文件（.atab 格式），包含比对细节。默认为 None。
            window (int): 用于长序列裁剪预测的窗口大小，默认为150个氨基酸。
            shift (int): 窗口滑动的步长，默认为50个氨基酸。shift < window 意味着窗口之间有重叠。
        """
