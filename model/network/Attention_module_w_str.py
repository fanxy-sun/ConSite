...
from model.network.Transformer import _get_clones
from model.network import Transformer
from model.network.resnet import ResidualNetwork
from model.network.SE3_network import SE3Transformer
from model.network.InitStrGenerator import InitStr_Network
import dgl
...
def make_graph(xyz, pair, idx, seq_mask, top_k=64, kmin=9):
    ...
    # 使用 seq_mask 确保不会从padding的残基创建边
    cond_mask = (seq_mask.unsqueeze(1) * seq_mask.unsqueeze(2)).bool()
    cond = cond & cond_mask
...
class CoevolExtractor(nn.Module):
    ...
    def forward(self, x_down, x_down_w, pair_mask=None):
        ...
        if pair_mask is not None:
            # pair_mask (B, L, L) -> (B, L, L, 1)，以便与pair特征进行广播相乘
...
class MSA2Pair(nn.Module):
    ...
    def forward(self, msa, pair_orig, att, seq_mask, msa_mask):
        ...
        # get sequence weight
        x_down = self.norm_down(x_down) # (B, N, L, n_feat_proj)
        w_seq = self.encoder(x_down, msa_mask=msa_mask).reshape(B, L, 1, N).permute(0,3,1,2)
        feat_1d = w_seq*x_down  # feat_1d(B, N, L, n_feat_proj)
        
        pair_mask = seq_mask.unsqueeze(1) * seq_mask.unsqueeze(2)
        pair = self.coevol(x_down, feat_1d, pair_mask=pair_mask)    # pair: (B, L, L, n_feat_out)

        # average pooling over N of given MSA info
        feat_1d = feat_1d.sum(1)    # (B, L, n_feat_proj)
        
        # query sequence info
        query = x_down[:,0] # (B,L,K)
        feat_1d = torch.cat((feat_1d, query), dim=-1) # feat_1d: (B, L, 2 * n_feat_proj)
        # tile 1D features
        left = feat_1d.unsqueeze(2).repeat(1, 1, L, 1)  # left (平铺后): (B, L, L, 2 * n_feat_proj)
        right = feat_1d.unsqueeze(1).repeat(1, L, 1, 1) # right (平铺后): (B, L, L, 2 * n_feat_proj)
        # update original pair features through convolutions after concat
        pair_orig = self.norm_orig(pair_orig)
        pair = self.norm_new(pair)
        pair = torch.cat((pair_orig, pair, left, right, att), -1)   # (B, L, L, C_in)
        pair = pair.permute(0,3,1,2).contiguous() # prep for convolution layer (B, C_in, L, L)
        pair = self.update(pair, mask=pair_mask.unsqueeze(1))    # (B, n_feat_out, L, L)
        pair = pair.permute(0,2,3,1).contiguous() # (B, L, L, n_feat_out)
...
class MSA2MSA(nn.Module):
    ...
    def forward(self, x, seq_mask=None, msa_mask=None, return_att=False):
        """
        Args:
            x (torch.Tensor): 输入MSA特征, shape (B, N, L, d_msa)
            seq_mask (torch.Tensor, optional): 序列长度掩码 (L-dim), shape (B, L).
            msa_mask (torch.Tensor, optional): MSA深度掩码 (N-dim), shape (B, N).
        ...
        """
        # attention along L
        x, att = self.encoder_1(x, seq_mask=seq_mask, msa_mask=msa_mask, return_att=True)

        # attention along N
        x = x.permute(0,2,1,3).contiguous() # shape: (B, L, N, d_msa)
        x = self.encoder_2(x, seq_mask=msa_mask)    # # 对于列注意力, 序列维度是 N。因此, 我们将 msa_mask 作为 seq_mask 参数传递给编码器。
...
class Pair2MSA(nn.Module):
    ...
    def forward(self, pair, msa, pair_mask):
        out = self.encoder(pair, msa, mask=pair_mask) # (B, N, L, K)
        return out
...
class Pair2Pair(nn.Module):
    ...
    def forward(self, x, pair_mask=None):
        """
        Args:
            x (torch.Tensor): 输入特征, shape (B, L, L, d_pair)
            pair_mask (torch.Tensor, optional): 掩码, shape (B, L, L). Defaults to None.
        """
        return self.encoder(x, mask=pair_mask)
...
class Str2Str(nn.Module):
    ...
    def forward(self, msa, pair, xyz, seq1hot, idx, seq_mask, msa_mask, top_k=64):
        # process msa & pair features
        B, N, L = msa.shape[:3]
        msa = self.norm_msa(msa)
        pair = self.norm_pair(pair)
        
        w_seq = self.encoder_seq(msa, msa_mask=msa_mask).reshape(B, L, 1, N).permute(0,3,1,2)
        msa = w_seq*msa
        msa = msa.sum(dim=1)
        msa = torch.cat((msa, seq1hot), dim=-1)
        msa = self.norm_node(self.embed_x(msa))
        if seq_mask is not None:
            msa = msa * seq_mask.unsqueeze(-1)

        pair = self.norm_edge(self.embed_e(pair))
        
        # define graph
        G = make_graph(xyz, pair, idx, seq_mask, top_k=top_k)
...
class Str2MSA(nn.Module):
    ...
    def forward(self, msa, xyz, state, seq_mask):
        ...
        # 结合序列掩码
        pair_mask = (seq_mask.unsqueeze(1) * seq_mask.unsqueeze(2)).unsqueeze(1) # (B, 1, L, L)
        mask_s = mask_s * pair_mask
        ...
        # msa: (B, N, L, d_msa), seq_mask: (B, L)
        # -> (B, L, 1) -> (B, 1, L, 1)
        final_mask = seq_mask.unsqueeze(1).unsqueeze(-1)
        msa = msa * final_mask
...
class IterBlock(nn.Module):
    ...
    def forward(self, msa, pair, seq_mask, msa_mask):
        """
        Args:
            msa (torch.Tensor): MSA特征, (B, N, L, d_msa)
            pair (torch.Tensor): 残基对特征, (B, L, L, d_pair)
            seq_mask (torch.Tensor): 序列掩码, (B, L)
            msa_mask (torch.Tensor): MSA深度掩码, (B, N)
        Returns:
        ...
        msa, att = self.msa2msa(msa, seq_mask=seq_mask, msa_mask=msa_mask)
        
        # 2. update pair features using given MSA
        pair = self.msa2pair(msa, pair, att, seq_mask=seq_mask, msa_mask=msa_mask)

        # 3. process pair features
        pair = self.pair2pair(pair, pair_mask=pair_mask)
        
        # 4. update MSA features using updated pair features
        msa = self.pair2msa(pair, msa, pair_mask=pair_mask)
...
class IterBlock_w_Str(nn.Module):
    ...
    def forward(self, msa, pair, xyz, seq1hot, idx, seq_mask, msa_mask, top_k=64):
        # $$是否应该继续使用self.str2str
        # $$IterBlock_w_Str的self.msa2msa, self.msa2pair, self.pair2pair, self.pair2msa与class IterBlock重复，是否应该去掉
        ...
        # 将seq_mask传递给str2str以构建正确的图
        xyz_new, state = self.str2str(msa.float(), pair.float(), xyz.float(), seq1hot, idx, seq_mask, msa_mask, top_k=top_k)
        # Str2MSA 使用距离掩码，但我们也需要结合序列掩码
        msa = self.str2msa(msa, xyz, state, seq_mask)
            
        return msa, pair, state, xyz_new
...
class FinalBlock(nn.Module):
    ...
    def forward(self, msa, pair, xyz, seq1hot, idx, seq_mask, msa_mask):
        pair_mask = seq_mask.unsqueeze(1) * seq_mask.unsqueeze(2)
        ...
        xyz_new, state = self.str2str(msa.float(), pair.float(), xyz.float(), seq1hot, idx, seq_mask, msa_mask, top_k=32)
        
        # lddt = self.pred_lddt(self.norm_state(state)) # (lddt预测已被移除)
...
class IterativeFeatureExtractor(nn.Module):
    ...
    # def forward(self, msa, pair, seq1hot, idx, xyz, seq_mask, msa_mask):
    #     """
    #     Args:
    #         msa (torch.Tensor): MSA特征, (B, N, L, d_msa)
    #         pair (torch.Tensor): Pair特征, (B, L, L, d_pair)
    #         seq1hot (torch.Tensor): 目标序列one-hot, (B, L, 21)
    #         idx (torch.Tensor): 残基索引, (B, L)
    #         xyz_in (torch.Tensor): 外部输入的坐标, (B, L, 3, 3)
    #         seq_mask (torch.Tensor): 序列长度掩码, (B, L)
    #         msa_mask (torch.Tensor): MSA深度掩码, (B, N)
    #     Returns:
    #         msa: 更新后的MSA特征, (B, N, L, d_msa)
    #         pair: 更新后的Pair特征, (B, L, L, d_pair)
    #         state: 结构状态特征, (B, L, d_state)
    #     """
        
    #     pair = self.initial(pair, pair_mask=(seq_mask.unsqueeze(1) * seq_mask.unsqueeze(2)))    # initial 是 Pair2Pair
    #     if self.n_module > 0:
    #         for i_m in range(self.n_module):
    #             # extract features from MSA & update original pair features
    #             msa, pair = self.iter_block_1[i_m](msa, pair, seq_mask=seq_mask, msa_mask=msa_mask)
       
    #     # 移除内部结构预测
    #     # xyz_pred = self.init_str(seq1hot, idx, msa, pair)

    #     top_ks = [128, 128, 64, 64]
    #     if self.n_module_str > 0:
    #         for i_m in range(self.n_module_str):
    #             msa, pair, state_str, _ = self.iter_block_2[i_m](msa, pair, xyz, seq1hot, idx, seq_mask=seq_mask, msa_mask=msa_mask, top_k=top_ks[i_m])   # 不使用 xyz_pred

    #     msa, pair, state = self.final(msa, pair, xyz, seq1hot, idx, seq_mask=seq_mask, msa_mask=msa_mask)
    #     return msa, pair, state
