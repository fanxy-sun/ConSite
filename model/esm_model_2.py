...
class ESM2(nn.Module):
    ...
    def _init_submodules(self):
        ...
        self.layers = nn.ModuleList(
            [
                # class: esm.modules.TransformerLayer
                TransformerLayer(
                    self.embed_dim,
                    4 * self.embed_dim,
...
def _load_model_and_alphabet_core_v2(model_data):
    ...
    # [function: esm.data.Alphabet.from_architecture] returns [class: esm.data.Alphabet] object.
    alphabet = esm.data.Alphabet.from_architecture("ESM-1b")

    # class: .model_structure.esm_layer.ESM2
    model = ESM2(
...
class EvolutionaryScaleModelingBeta(nn.Module):
    """
    ...
    def __init__(self, path, model="ESM-1b", readout="mean"):
        super(EvolutionaryScaleModelingBeta, self).__init__()
        ...
        for param in self.model.parameters():       # 默认冻结模型参数，不会在训练过程中更新，可通过main_train.py中的train_all_parameters参数调整
    ...
    def load_weight(self, path, model):
        ...
        # function: torchdrug.utils.data.download.download
        model_file = utils.download(self.url[model], path, md5=self.md5[model])

        # function: torch.serialization.load
    ...
    def forward(self, graph, input, all_loss=None, metric=None):
        ...
        Returns:
            dict with ``residue_feature`` and ``graph_feature`` fields:
                residue representations of shape: (|V_{res}|, d), |V_res| is the total number of amino acid residues across all proteins in the batch, d is the feature dimension, determined by the output_dim attribute of the EvolutionaryScaleModelingBeta class. Note that |V_res| denotes the total count of all genuinely existing amino acid residues across all proteins in a batch.
                graph representations of shape: (n, d), n is the total number of proteins in the batch, d is the feature dimension, determined by the output_dim attribute of the EvolutionaryScaleModelingBeta class.
        """

        # =========================================== Top@Process input of ESM======================================================
